...
(function($) {
    ...
    var Spur = function(element) {
        ...
        self.init = function(options) {
            ...
            self.el.addClass('spur-element spur-loading').height(self.o.height);

            if (typeof self.o.source === 'string') {
                // Loading .json file with AJAX
                $.getJSON(self.o.source, function(data) { // Success
                    processData(data);
                    self.el.removeClass('spur-loading');

                }).fail(function() { // Failure: couldn't load JSON file or it is invalid.
                    console.error('Couldn\'t load map data. (Make sure to run the script through web server)');
                    self.el.removeClass('spur-loading').addClass('spur-error');
        ...
        function Tooltip() {
            ...
            this.init = function() {
                ...
                // Construct
                this.el = $('<div></div>').addClass('spur-tooltip');
                this.close = $('<a></a>').addClass('spur-tooltip-close').attr('href', '#').appendTo(this.el);
                this.close.on('click touchend', function(e) {
                    e.preventDefault();
                    self.hideLocation();
                    if (!self.o.zoom || self.o.zoomoutclose) self.moveTo(0.5, 0.5, self.fitscale, 400, 'easeInOutCubic');
                });
                if (this.tooltipthumb) this.thumbnail = $('<img>').addClass('spur-tooltip-thumbnail').hide().appendTo(this.el);
                this.content = $('<div></div>').addClass('spur-tooltip-content').appendTo(this.el);
                this.title = $('<h4 ></h4>').addClass('spur-tooltip-title').appendTo(this.content);
                this.desc = $('<div></div>').addClass('spur-tooltip-description').appendTo(this.content);
                if (this.tooltiplink) this.link = $('<a>More</a>').addClass('spur-popup-link').attr('href', '#').hide().appendTo(this.el);
                this.triangle = $('<div></div>').addClass('spur-tooltip-triangle').prependTo(this.el);
            ...
            this.show = function(location) {
                ...
                if (location) {
                    ...
                    this.title.attr('data-ids', location.id);
                    this.title.off('click touchend').on('click touchend', function(e) {
                        e.preventDefault();
                        self.popupTitleClicked(location)
                    });
                    ...
                    // Shift
                    this.pin = $('.spur-pin[data-location="' + location.id + '"]');
                ...
                if (this.location) {
                    ...
                    if (self.o.smartip) {
                        ...
                        if (Math.abs(mt) > cy) {
                            ...
                            this.el.addClass('spur-bottom');
                        } else this.el.removeClass('spur-bottom');
        ...
        function Lightbox() {
            ...
            this.init = function() {
                // Construct
                this.el = $('<div></div>').addClass('spur-lightbox mfp-hide');
                this.title = $('<h2></h2>').addClass('spur-lightbox-title').appendTo(this.el);
                this.desc = $('<div></div>').addClass('spur-lightbox-description').appendTo(this.el);
                this.link = $('<a>More</a>').addClass('spur-popup-link').attr('href', '#').hide().appendTo(this.el);

                // Popup Image
                $('body').magnificPopup({
                    delegate: '.spur-popup-image',
                    type: 'image',
                    removalDelay: 300,
                    mainClass: 'mfp-fade'
                });
            ...
            this.show = function(location) {
                this.location = location;
                //--this.link.attr('data-id', location.id);
                this.title.text(location.title);
                this.desc.html(location.description);

                if (location.link) this.link.attr('href', location.link).attr('target', '_blank').show();
                else this.link.hide();
        ...
        function HoverTooltip() {
            ...
            this.init = function() {
                ...
                // Construct
                this.el = $('<div></div>').addClass('spur-tooltip spur-hovertip');
                this.title = $('<h4></h4>').addClass('spur-tooltip-title').appendTo(this.el);
                if (this.hovertipdesc) this.desc = $('<div></div>').addClass('spur-tooltip-description').appendTo(this.el);
                this.triangle = $('<div></div>').addClass('spur-tooltip-triangle').appendTo(this.el);

                // Events 
                // pins + old svg
                $(self.map).on('mouseover', '.spur-layer a', function() {
                    var id = '';
                    if ($(this).hasClass('spur-pin')) {
                        id = $(this).data('location');
                        s.pin = $('.spur-pin[data-location="' + id + '"]');
        ...
        function Deeplinking() {
            ...
            this.update = function(id) {
                var url = window.location.protocol + "//" + window.location.host + window.location.pathname + '#/maps?' + this.param + '=' + id;
        ...
        function Minimap() {
            ...
            this.init = function() {
                this.el = $('<div></div>').addClass('spur-minimap').appendTo(self.container);
            ...
            this.addLayer = function(data) {
                var layer = $('<div></div>').addClass('spur-minimap-layer').attr('data-floor', data.id).appendTo(this.el),
                    s = this;

                $('<img>').attr('src', data.minimap).addClass('spur-minimap-background').appendTo(layer);
                $('<div></div>').addClass('spur-minimap-overlay').appendTo(layer);
                $('<img>').attr('src', data.minimap).addClass('spur-minimap-active').on('load', function() {
                    s.update();
                    $(this).addClass('spur-clip-transition');
                }).appendTo(layer);
            }

            this.show = function(target) {
                $('.spur-minimap-layer', this.el).hide();
                $('.spur-minimap-layer[data-floor="' + target + '"]', this.el).show();
            }

            this.update = function(x, y) {
                var active = $('.spur-minimap-active', this.el);
        ...
        function Sidebar() {
            ...
            this.init = function() {
                ...
                this.el = $('<div></div>').addClass('spur-sidebar').appendTo(self.el);

                if (self.o.search) {
                    var form = $('<form></form>').addClass('spur-search-form').submit(function() {
                        return false;
                    }).appendTo(this.el);
                    self.clear = $('<button></button>').attr('type', 'button').addClass('spur-search-clear').click(function() {
                        input.val('');
                        input.keyup();
                        self.clear.fadeOut(100);
                    }).appendTo(form);
                    var input = $('<input>').attr({ 'type': 'text', 'spellcheck': 'false', 'placeholder': 'Search...' }).addClass('spur-search-input').keyup(function() {
                        var keyword = $(this).val();
                        s.search(keyword);
                    }).prependTo(form);
                }

                var listContainer = $('<div></div>').addClass('spur-list-container').appendTo(this.el);
                this.list = $('<ol></ol>').addClass('spur-list').appendTo(listContainer);
                this.notfound = $('<p></p>').addClass('spur-not-found').text('Nothing found. Please try a different search.').appendTo(listContainer);
                //if (this.list == null) { listContainer.css('display', 'none') } else { listContainer.css('display', 'block') }
            ...
            this.addCategories = function(categories) {
                ...
                if (categories) {
                    $.each(categories, function(index, category) {
                        var item = $('<li></li>').addClass('spur-list-category').attr('data-category', category.id);
                        var ol = $('<ol></ol>').css('border-color', category.color).appendTo(item);
                        if (category.show == 'false') ol.hide();
                        else item.addClass('spur-opened');
                        var link = $('<a></a>').attr('href', '#').attr('title', category.title).css('background-color', category.color).text(category.title).prependTo(item);
                        link.on('click', function(e) {
                            e.preventDefault();
                            item.toggleClass('spur-opened');
                            ol.slideToggle(200);
                        });
                        if (category.icon) $('<img>').attr('src', category.icon).addClass('spur-list-thumbnail').prependTo(link);
                        $('<span></span>').text('0').addClass('spur-list-count').prependTo(link);
                        list.append(item);
                    });
                }
            }
            ...
            this.addLocation = function(data) {
                var item = $('<li></li>').addClass('spur-list-location').addClass('spur-list-shown').attr('data-location', data.id);
                ...
                if (data.thumbnail) $('<img>').attr('src', data.thumbnail).addClass('spur-list-thumbnail').appendTo(link).hide();
                $('<h4></h4>').text(data.title).appendTo(link)
                $('<span></span>').html(data.about).appendTo(link);
                var category = $('.spur-list-category[data-category="' + data.category + '"]');

                if (category.length) $('ol', category).append(item);
                else this.list.append(item);

                // Count
                $('.spur-list-count', category).text($('.spur-list-shown', category).length);
            }
            ...
            this.search = function(keyword) {
                ...
                if (keyword) self.clear.fadeIn(100);
                    ...
                    self.clear.fadeOut(100);
                    $(this).removeClass('spur-list-shown');
                    $(this).slideUp(200);
                }

                $('.spur-list li', self.el).each(function() {
                    ...
                    if ($(this).text().search(new RegExp(keyword, "i")) <= 0) {
                        ...
                        $(this).addClass('spur-list-shown');
                        $(this).show();
                    }
                });

                $('.spur-list > li', self.el).each(function() {
                    var count = $('.spur-list-shown', this).length;
                    $('.spur-list-count', this).text(count);
                });

                // Show not-found text
                if ($('.spur-list > li.spur-list-shown').length > 0) {
        ...
        function DevTools() {
            ...
            this.init = function() {
                this.el = $('<div></div>').addClass('spur-coordinates').appendTo(self.container);
                this.el.append('x: ');
                $('<code></code>').addClass('spur-coordinates-x').appendTo(this.el);
                this.el.append(' y: ');
                $('<code></code>').addClass('spur-coordinates-y').appendTo(this.el);

                $('.spur-layer', self.map).on('mousemove', function(e) {
                    var x = (e.pageX - self.map.offset().left) / self.map.width(),
                        y = (e.pageY - self.map.offset().top) / self.map.height();
                    $('.spur-coordinates-x').text(parseFloat(x).toFixed(4));
                    $('.spur-coordinates-y').text(parseFloat(y).toFixed(4));
                });
        ...
        function ClearButton() {
            ...
            this.init = function() {
                this.el = $('<a></a>').attr('href', '#').addClass('spur-clear-button').appendTo(self.container);
        ...
        function ZoomButtons() {
            ...
            this.init = function() {
                this.el = $('<div></div>').addClass('spur-zoom-buttons').appendTo(self.container);

                if (!self.o.clearbutton) this.el.css('bottom', '0');

                // Zoom in button
                this.zoomin = $('<a></ha>').attr('href', '#').addClass('spur-zoomin-button').appendTo(this.el);
                ...
                // Zoom out button
                this.zoomout = $('<a></ha>').attr('href', '#').addClass('spur-zoomout-button').appendTo(this.el);
            ...
            this.update = function(scale) {
                this.zoomin.removeClass('spur-disabled');
                this.zoomout.removeClass('spur-disabled');
                if (scale == self.fitscale) this.zoomout.addClass('spur-disabled');
                else if (scale == self.o.maxscale) this.zoomin.addClass('spur-disabled');
            }
        }
        ...
        function Fullscreen() {
            this.el = null;
            self.el.toggleClass('spur-fullscreen');
            this.init = function() {
                // Fullscreen Button
                $('<a></a>').attr('href', '#').attr('href', '#').addClass('spur-fullscreen-button').click(function(e) {
                    e.preventDefault();
                    self.el.toggleClass('spur-fullscreen');
                    $(document).resize();
                ...
                $(document).keyup(function(e) {
                    if (e.keyCode === 27) {
                        $('.spur-element.spur-fullscreen').removeClass('spur-fullscreen');
                        $(document).resize();
                    }
                });
            }
        ...
        var processData = function(data) {
            ...
            self.container = $('<div></div>').addClass('spur-container').appendTo(self.el);
            self.map = $('<div></div>').addClass('spur-map').appendTo(self.container);
            if (self.o.zoom) self.map.addClass('spur-zoomable');

            self.levelselect = $('<select></select>').addClass('spur-levels-select');
            ...
            if (data.levels) {
                $.each(data.levels, function(index, level) {
                    ...
                    var layer = $('<div></div>').addClass('spur-layer').attr('data-floor', level.id).hide().appendTo(self.map);
                    switch (extension) {
                        ...
                        case 'png':
                        case 'gif':
                            $('<img>').attr('src', source).addClass('spur-map-image').appendTo(layer);
                            break;

                            // Vector format
                        case 'svg':
                            $('<div></div>').addClass('spur-map-image').load(source, function() {
                                // Setting up the locations on the map
                                $(self.o.selector, this).each(function() {
                                    var location = self.getLocationData($(this).attr('id'));
                                    if (location) {
                                        $(this).attr('class', 'spur-clickable');
                                        location.el = $(this);
                                        ...
                                        // Landmark mode
                                        if (self.o.landmark === location.id) $(this).attr('class', 'spur-active');
                                    }
                                });
                    ...
                    $.each(level.locations, function(index, location) {
                        ...
                        if (location.pin.indexOf('hidden') == -1) {
                            if (self.o.markers) {
                                var pin = $('<a></a>').attr('href', '#').addClass('spur-pin').css({ 'top': top + '%', 'left': left + '%' }).appendTo(layer);
            ...
            // Fullscreen
            if (self.o.fullscreen) self.fullscreen = self.fullscreen;
            //new Fullscreen().init();
            ...
            // Level switcher
            if (nrlevels > 1) {
                self.levels = $('<div></div>').addClass('spur-levels');
                var up = $('<a href="#"></a>').addClass('spur-levels-up').appendTo(self.levels);
                self.levelselect.appendTo(self.levels);
                var down = $('<a href="#"></a>').addClass('spur-levels-down').appendTo(self.levels);
                ...
                up.click(function(e) {
                    e.preventDefault();
                    if (!$(this).hasClass('spur-disabled')) self.switchLevel('+');
                });
            ...
            $(window).resize(function() {
                // Mobile
                if ($(window).width() < 668) {
                    if (self.el.hasClass('spur-fullscreen')) self.container.height($(window).height());
                    else {
        ...
        var addControls = function() {
            var map = self.map,
                mapbody = $('.spur-map-image', self.map);
            ...
            mapbody.on('mousedown', function(e) {
                ...
                $(document).on('mouseup', function() {
                    ...
                    map.removeClass('spur-dragging');
                });
            });

            // Double click
            self.el.on('dblclick', '.spur-map-image', function(e) {
                e.preventDefault();
            ...
            if (self.o.mousewheel) {
                $('.spur-layer', self.el).bind('mousewheel DOMMouseScroll', function(e, delta) {
                    var scale = self.scale;
        ...
        self.switchLevel = function(target) {
            ...
            var layer = $('.spur-layer[data-floor="' + target + '"]', self.el);
            ...
            // Show target layer
            $('.spur-layer:visible', self.map).hide();
            layer.show();
            ...
            var index = self.levelselect.get(0).selectedIndex,
                up = $('.spur-levels-up', self.el),
                down = $('.spur-levels-down', self.el);

            up.removeClass('spur-disabled');
            down.removeClass('spur-disabled');
            if (index == 0) up.addClass('spur-disabled');
            else if (index == self.levelselect.get(0).length - 1) down.addClass('spur-disabled');
        ...
        self.popupTitleClicked = function(id) {
            self.el.trigger('popupClicked', id);
        }
        self.showYAH = function(id) {
            ...
            $.each(self.data.levels, function(index, level) {
                $.each(level.locations, function(index, location) {
                    if (location.id == id) {
                        ...
                        //     cy: location.cy
                        // });
                        $("#yah").attr("transform", "translate(" + location.cx + " " + location.cy + ")");
                        /*console.log(self.scale)
                        console.log('scale * yy', (self.scale * location.yy * 3))
                        var xx = location.x.split('.');
                        var yy = location.y.split('.');
                        var x = xx[1].toString().replace(/\B(?=(\d{2})+(?!\d))/g, ".");
                        var y = yy[1].toString().replace(/\B(?=(\d{2})+(?!\d))/g, ".");
                        // var a = y.split('.');
                        // console.log(a[0])
                        // console.log(parseInt(a[0]) + 2);
                        $("#youarehere").css({ top: y + "%", left: x + "%" });*/
                        //$("#youarehere").css({ top: (self.scale * location.y) + "px", left: (self.scale * location.x) + "px" });
                    }
                });
            });
            //$("#youarehere").css({ top: "40%", left: "20%" });
        }
                ...
                $.each(level.locations, function(index, location) {
                    if (location.id == id) {
                        ...
                        switch (action) {
                            ...
                            case 'select':
                                if (location.el) {
                                    if (hasClass(location.el, 'spur-active')) removeClass(location.el, 'spur-active');
                                    else addClass(location.el, 'spur-active');
                                }
                                return false;
                            case 'none':
                                var zoom = location.zoom ? parseFloat(location.zoom) : self.o.maxscale;
                        ...
                        // Active state
                        removeClass($('.spur-active', self.el), 'spur-active');
                        if (location.el) addClass(location.el, 'spur-active');
    ...
    // jQuery Plugin
    $.fn.spur = function(options) {

        return this.each(function() {
            var element = $(this);

            // Plugin already initiated on element
            if (element.data('spur')) return;

            var instance = (new Spur(element)).init(options);

            // Store plugin object in element's data
            element.data('spur', instance);
        });
    };
